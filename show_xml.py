...
def extract_music(soup):
    ...
    path = '/home/franklin/Desktop/xml2vec-master/test.txt'
    open(path, 'r+').truncate()   #清空文件内容
    ...
    print ("into extract_music")
    # パートごとに

    print ("*******************************************************************")
    # print (soup.find_all("part"))
    print ("*******************************************************************")
                ...
                for content in m.find_all({"attributes", "direction", "harmony", "note", "backup"}):
                    ...
                    if content.name == "attributes":
                        ...
                        if content.find("divisions"): # divisions
                            ...
                            note_divisions = int(content.divisions.string) #找到division值记录下来
                    ...
                    if content.name == "note" and not content.chord and content.duration \
                        ...
                        rate = float(content.duration.string) / note_divisions
                        if rate >= 0.0625 and rate < 0.125:  # 長さが64分音符以上32分音符未満
                            note_type =  "64th"
                        elif rate >= 0.125 and rate < 0.25:  # 32分音符以上16分音符未満
                            note_type =  "32nd"
                        ...
                        if content.pitch:
                            ...
                            if note_step == 'C':
                                pitch_num = note_oct*100+10+note_alt
                            elif note_step == 'D':
                                pitch_num = note_oct*100+20+note_alt
                            elif note_step == 'E':
                                pitch_num = note_oct*100+30+note_alt
                            elif note_step == 'F':
                                pitch_num = note_oct*100+40+note_alt
                            elif note_step == 'G':
                                pitch_num = note_oct*100+50+note_alt
                            elif note_step == 'A':
                                pitch_num = note_oct*100+60+note_alt
                            elif note_step == 'B':
                                pitch_num = note_oct*100+70+note_alt
                        ...
                        f_name = open(path,'a')
                        f_name.write(str(pitch_num)+'\t'+str(note_type)+'\t'+str(note_tie)+'\t'+str(dot)+'\n')
                        f_name.close()
...
def finalize(score):
    ...
    rough_string = WriteHeader() + rough_string
...
if __name__ == "__main__":
    ...
    piece_info, melody, chords = extract_music(soup)
    ...
    WriteScore(score, piece_info, melody, chords)
